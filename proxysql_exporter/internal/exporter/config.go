...
import (
	"proxysql_exporter/pkg/logger"
	"proxysql_exporter/pkg/utils"
...
var (
	...
	DefaultConfig = Config{
		Logging: logger.Config{
			Level:   "debug",
			LogPath: "/var/log/uos-exporter/proxysql-exporter.log",
			MaxSize: "10MB",
...
func init() {
	...
	Configfile = kingpin.Flag("config", "Configuration file").
		Short('c').
		Default("/etc/uos-exporter/proxysql-exporter.yaml").
		String()
}
