...
func initSNMP(logger *logrus.Entry) (string, error) {
	// 初始化环境变量
	if err := initMIBsEnvironment(); err != nil {
		return "", err
	}

	// 设置MIB目录和选项
	setupMIBSettings(logger)

	// 重定向stderr以捕获输出
	output, err := captureStderrOutput()
	if err != nil {
		return "", err
	}
...
// helper functions
func initMIBsEnvironment() error {
	return os.Setenv("MIBS", "ALL")
}

func setupMIBSettings(logger *logrus.Entry) {
...
func captureStderrOutput() (string, error) {
	r, w, err := os.Pipe()
	if err != nil {
...
func buildMIBTree(t *C.struct_tree, n *Node, oid string) {
	// 设置节点基础信息
	setNodeBasicInfo(t, n, oid)

	// 设置节点类型和访问权限
	setNodeTypeAndAccess(t, n)

	// 设置节点额外属性
	setNodeAdditionalAttributes(t, n)

	// 处理枚举值
	setNodeEnumValues(t, n)

	// 处理子节点
	if t.child_list != nil {
		processChildNodes(t, n)
	}
...
// 辅助函数
func setNodeBasicInfo(t *C.struct_tree, n *Node, oid string) {
...
func setNodeTypeAndAccess(t *C.struct_tree, n *Node) {
...
func setNodeAdditionalAttributes(t *C.struct_tree, n *Node) {
...
func setNodeEnumValues(t *C.struct_tree, n *Node) {
	n.EnumValues = make(map[int]string)
	enum := t.enums
	for enum != nil {
...
func processChildNodes(t *C.struct_tree, n *Node) {
	head := t.child_list
	n.Children = []*Node{}
	for head != nil {
		child := &Node{}
...
func setNodeIndexes(t *C.struct_tree, n *Node) {
	index := t.indexes
	indexes := []string{}
	for index != nil {
