...
func TestGenerateConfigModule(t *testing.T) {
	var regexpFooBar metrics.Regexp
	regexpFooBar.Regexp, _ = regexp.Compile(".*")

	strMetrics := make(map[string][]metrics.RegexpExtract)
	strMetrics["Status"] = []metrics.RegexpExtract{
		{
			Regex: regexpFooBar,
			Value: "5",
		},
	...
	cases := []struct {
		node *Node
		cfg  *ModuleConfig   // SNMP generator metrics.
		out  *metrics.Module // SNMP exporter metrics.
	}{
	...
	for i, c := range cases {
		...
		for _, m := range c.out.Metrics {
			if m.Indexes == nil {
				m.Indexes = []*metrics.Index{}
			}
			if m.Lookups == nil {
				m.Lookups = []*metrics.Lookup{}
			}
		}

		nameToNode := prepareTree(c.node, logger)
		// got, err := generateConfigModule(c.cfg, c.node, nameToNode, promslog.NewNopLogger())
		got, err := generateConfigModule(c.cfg, c.node, nameToNode, logger)
		if err != nil {
