...
import (
	...
	"snmp_exporter/internal/exporter"
	metrics "snmp_exporter/internal/metrics"
	"snmp_exporter/pkg/logger"
	"snmp_exporter/pkg/ratelimit"
	"snmp_exporter/pkg/utils"
...
func (s *Server) setupHttpServer() error {
	...
	mux.HandleFunc("/-/reload", metrics.UpdateConfiguration) // Endpoint to reload configuration.
	...
	mux.Handle(s.CommonConfig.MetricsPath, promhttp.Handler())
	...
	landPage, err := web.NewLandingPage(landConfig)
	if err != nil {
