...
func (s *Server) registerMetrics() {
    // 注册业务指标收集器
    s.promReg.MustRegister(metrics.NewCollector(s.client, *scrapeTimeout, *enableRemote))
}
func (s *Server) SetUp() error {
	...
	if config.ScrapeUrl != nil {
        ...
        // 构建 Paperless 客户端
        // var err error
        if s.client, err = clientFlags.Build(); err != nil {
...
var (
    ...
    enableRemote   = kingpin.Flag("enable-remote", "Enable remote API calls").Bool()
    scrapeTimeout  = kingpin.Flag("scrape-timeout", "Scrape timeout").Default("1m").Duration()
)
