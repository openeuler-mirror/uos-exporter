...
import (
	"context"
	"docker_swarm_exporter/config"
	"docker_swarm_exporter/internal/exporter"
	"docker_swarm_exporter/internal/metrics"
	_ "docker_swarm_exporter/internal/metrics"
	"docker_swarm_exporter/pkg/logger"
	"docker_swarm_exporter/pkg/ratelimit"
	"docker_swarm_exporter/pkg/utils"
	"fmt"
...
func (s *Server) SetUp() error {
	...
	s.ExporterConfig = config.DefaultSettings
	err = exporter.Unpack(&s.ExporterConfig)
	if err != nil {
		logrus.Warnf("Failed to unpack config: %s", err)
		logrus.Info("Use default config")
		logrus.Infof("swarm host: %s\n", config.DefaultSettings.SwarmHost)
		s.ExporterConfig = config.DefaultSettings
	}
	//fmt.Printf("wd: %#v\n", s.ExporterConfig)
	err = metrics.ConnectSwarm(s.ExporterConfig.SwarmHost)
	if err != nil {
		logrus.Errorf("Connect swarm error: %v", err)
		return err
	}
	return nil
}
...
func (s *Server) Stop() {
	...
	logrus.Info("disconnect swarm")
	metrics.DisconnectSwarm()
