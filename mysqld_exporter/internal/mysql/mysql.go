...
func NewInstance(dsn string) (*Instance, error) {
	...
	if err := db.Ping(); err != nil {
                if closeErr := db.Close(); closeErr != nil {
                        return nil, fmt.Errorf("database ping failed: %w (close also failed: %v)", err, closeErr)
                }
	...
	if err != nil {
                if closeErr := db.Close(); closeErr != nil {
                        return nil, fmt.Errorf("failed to get database version: %w (close also failed: %v)", err, closeErr)
                }
	...
	if err != nil {
                if closeErr := db.Close(); closeErr != nil {
                        return nil, fmt.Errorf("failed to parse version: %w (close also failed: %v)", err, closeErr)
                }
