...
import (
	...
	"openvpn_exporter/internal/metrics"
	"openvpn_exporter/pkg/logger"
	"openvpn_exporter/pkg/ratelimit"
	"openvpn_exporter/pkg/utils"
	"os"
	"sync"
	"time"
...
func (s *Server) SetUp() error {
	...
	// 加载OpenVPN相关配置
	err = config.LoadConfigFile(*exporter.Configfile)
	if err != nil {
		logrus.Warnf("Failed to load OpenVPN config: %v", err)
	}

	// 注册OpenVPN收集器（在配置加载完成后）
	metrics.RegisterOpenVPNCollector()
