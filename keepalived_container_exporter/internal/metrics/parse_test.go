...
func TestGetIntStatus(t *testing.T) {
	...
	script := StateConverter{}

	for expected, status := range acceptableStatuses {
		// script.Status = status
		if result, ok := script.ScriptStatusToInt(status); !ok || result != expected {
			t.Fail()
		}
	}

	status := "NOTGOOD"
	if result, ok := script.ScriptStatusToInt(status); ok || result != -1 {
		t.Fail()
	}
}
...
func TestGetIntState(t *testing.T) {
	...
	for expected, state := range acceptableStates {
		// script.State = state
		if result, ok := script.ScriptStateToInt(state); !ok || result != expected {
			t.Fail()
		}
	}

	state := "NOTGOOD"
	if result, ok := script.ScriptStateToInt(state); ok || result != -1 {
		t.Fail()
	}
}
...
func TestVRRPDataStringToIntState(t *testing.T) {
	...
	for expected, state := range acceptableStates {
		result, ok := script.VRRPStateToInt(state)
		if !ok || result != expected {
			t.Fail()
		}
	}

	result, ok := script.VRRPStateToInt("NOGOOD")
	if ok || result != -1 {
		t.Fail()
	}
}
...
func TestV215ParseVRRPScript(t *testing.T) {
	...
	vrrpScripts := tp.ParseVRRPScript(f)
