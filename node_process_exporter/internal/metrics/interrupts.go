...
func init() {
	exporter.Register(NewInterruptsCollector())
}
...
func NewInterruptsCollector() *interruptsCollector {
	logger := slog.Default()
	
	// 默认配置：排除空模式，包含所有中断，包含零值
	nameFilter := newDeviceFilter("", "")
	includeZeros := true

	return &interruptsCollector{
		baseMetrics: NewMetrics("node_interrupts_collect_errors_total", "Number of errors that occurred during interrupts collection", []string{}),
...
func (c *interruptsCollector) Collect(ch chan<- prometheus.Metric) {
	if err := c.Update(ch); err != nil {
		c.logger.Error("Error updating interrupts metrics", "error", err)
...
func (c *interruptsCollector) Update(ch chan<- prometheus.Metric) error {
	interrupts, err := c.getInterrupts()
	if err != nil {
...
func (c *interruptsCollector) getInterrupts() (map[string]interrupt, error) {
	file, err := os.Open(c.procFilePath("interrupts"))
	if err != nil {
		return nil, err
	}
	defer file.Close()

	return c.parseInterrupts(file)
}

func (c *interruptsCollector) parseInterrupts(r io.Reader) (map[string]interrupt, error) {
	var (
...
func (c *interruptsCollector) procFilePath(name string) string {
