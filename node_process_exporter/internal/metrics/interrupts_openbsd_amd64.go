...
// _C_int is a workaround for the package syscall not providing a type to
// match C's int, which is the typical type for sysctl arguments.
type _C_int int32

func sysctl(mib []_C_int) ([]byte, error) {
	// We use a zero-sized buffer here, which will be ignored by
	// unix.SysctlRaw. The actual buffer will be allocated
	// inside unix.SysctlRaw.
	return unix.SysctlRaw(mib, nil)
}
...
func intr(idx _C_int) (itr interruptOpenbsd, err error) {
	...
	// Convert bytes to string safely by stopping at the first null byte
	itr.device = bytesToString(dev[:])
...
type interruptOpenbsd struct {
	vector int
	device string
	values []float64
}

func getInterrupts() (map[string]interruptOpenbsd, error) {
	var interrupts = map[string]interruptOpenbsd{}
	n := nintr()
