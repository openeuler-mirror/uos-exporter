...
// computePrometheusInfo Calculates the name, description, tag key, and tag value of the Prometheus indicator
func computePrometheusInfo(vName, vGroup, vIdentifier, vDescription string) (name, description string, labelKeys, labelValues []string) {
	// Handle the case of Varnish >= 5.2, parsing the identifiers in vName
	handleVarnish52Identifier(&vIdentifier, vName)

	// Generate indicator names and descriptions
	name, description = generatePrometheusNameAndDescription(vName, vGroup, vIdentifier, vDescription)

	// Processing labels: The final indicator name and description may be modified
	handleLabels(&name, &description, &labelKeys, &labelValues, vName, vIdentifier)

	return name, description, labelKeys, labelValues
}

// handleVarnish52Identifier Handle identifiers for Varnish >= 5.2
func handleVarnish52Identifier(vIdentifier *string, vName string) {
...
// generatePrometheusNameAndDescription Generate a name and description for the Prometheus indicator
func generatePrometheusNameAndDescription(vName, vGroup, vIdentifier, vDescription string) (name, description string) {
...
// handleLabels Processing tags may modify the final indicator name and description
func handleLabels(name, description *string, labelKeys, labelValues *[]string, vName, vIdentifier string) {
	if len(vIdentifier) > 0 {
		handleBackendLabels(labelKeys, labelValues, vName, vIdentifier)
		handleDefaultLabels(labelKeys, labelValues, *name, vIdentifier)
	}

	handleGroupings(name, description, labelKeys, labelValues)
}

// handleBackendLabels Processing back-end labels
func handleBackendLabels(labelKeys, labelValues *[]string, vName, vIdentifier string) {
...
// handleDefaultLabels Handle default tags
func handleDefaultLabels(labelKeys, labelValues *[]string, name, vIdentifier string) {
	if len(*labelKeys) == 0 {
...
// handleGroupings Processing grouping logic
func handleGroupings(name, description *string, labelKeys, labelValues *[]string) {
