...
func init() {
	exporter.Register(NewHwmonCollector())
}
...
func NewHwmonCollector() *HwmonCollector {
	return &HwmonCollector{
		NewMetrics("node_hwmon_collector", "Hardware monitoring collector metrics", []string{}),
	}
}
...
func sysReadFile(file string) ([]byte, error) {
	...
	if !strings.HasPrefix(cleanFile, "/sys/class") {
		return nil, fmt.Errorf("unallowed path: %s", cleanFile)
	}
...
func (c *HwmonCollector) hwmonName(dir string) (string, error) {
	...
	if !strings.HasPrefix(cleanPath, "/sys/class/") {
		return "", fmt.Errorf("unallowed path: %s", cleanPath)
	}
	sysnameRaw, nameErr := os.ReadFile(cleanPath)
