...
func (s *Server) SetUp() error {
	...
	// 加载LDAP配置
	err = s.loadLDAPConfig()
	if err != nil {
		logrus.Errorf("Loading LDAP config failed: %v", err)
		return err
	}
...
func (s *Server) loadLDAPConfig() error {
	cfg, err := config.LoadConfig(*exporter.Configfile)
	if err != nil {
		logrus.Warnf("Failed to load LDAP config from file: %v", err)
		logrus.Info("Using default LDAP config")
		defaultCfg := config.DefaultConfig()
		cfg = &defaultCfg
	}

	// 初始化基础LDAP健康检查导出器配置 (替代cn=Monitor导出器)
	err = metrics.InitializeBasicLDAPWithConfig(cfg)
	if err != nil {
		logrus.Errorf("Failed to initialize Basic LDAP exporter with config: %v", err)
		return err
	}

	// cn=Monitor导出器
	err = metrics.InitializeWithConfig(cfg)
	if err != nil {
		logrus.Errorf("Failed to initialize LDAP exporter with config: %v", err)
		return err
	}

	logrus.Info("LDAP config loaded successfully")
	return nil
}
