...
import (
	...
	"strings"
	"sync"
	"systemd_exporter/config"
	"systemd_exporter/internal/exporter"
	_ "systemd_exporter/internal/metrics"
	"systemd_exporter/pkg/logger"
	"systemd_exporter/pkg/ratelimit"
...
func (s *Server) loadConfig() error {
	...
	if !utils.FileExists(configPath) {
		alternativePaths := []string{
			"/etc/uos-exporter/systemd-exporter.yml",
			"/etc/uos-exporter/systemd-exporter.yaml",
		}

		for _, path := range alternativePaths {
			if utils.FileExists(path) {
				configPath = path
				logrus.Infof("Using alternative config file: %s", path)
				break
			}
		}
	}
	...
	if utils.FileExists(configPath) {
		...
		if !strings.HasPrefix(cleanPath, "/etc/uos-exporter/") {
			logrus.Errorf("Invalid config path: %s", configPath)
			logrus.Info("Use default config")
			return nil
		}
		...
		logrus.Infof("Loaded config file from: %s", configPath)
		logrus.Info("CommonConfig file loaded")
		return nil
	}

	// 没有找到配置文件，使用默认配置
	logrus.Errorf("Config file not found: %s", configPath)
	logrus.Info("Use default config")
	return nil
}
