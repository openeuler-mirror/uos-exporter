...
func (s *Server) setupLog() error {
	...
	maxAge := time.Duration(s.CommonConfig.Logging.MaxAge) * 24 * time.Hour
	logConfig := logger.NewConfig(s.CommonConfig.Logging.Level, s.CommonConfig.Logging.LogPath, safeUint64ToInt64(size), maxAge)
...
func (s *Server) loadConfig() error {
	...
	if configPath == "" {
		configPath = "/etc/uos-exporter/node-network-exporter.yaml"
	}
