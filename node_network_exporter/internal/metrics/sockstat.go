...
func init() {
	exporter.Register(NewSockStatCollector())
}
...
func (c *sockStatCollector) Collect(ch chan<- prometheus.Metric) {
	...
	if err != nil {
		c.logger.Error("failed to open procfs", "error", err)
		return
	}
	...
	default:
		c.logger.Error("failed to get IPv4 sockstat data", "error", err)
		return
	}
	...
	default:
		c.logger.Error("failed to get IPv6 sockstat data", "error", err)
		return
	}
...
func (c *sockStatCollector) update(ch chan<- prometheus.Metric, isIPv6 bool, s *procfs.NetSockstat) {
	...
	if !isIPv6 && s.Used != nil {
		...
		ch <- prometheus.MustNewConstMetric(
			prometheus.NewDesc(
				prometheus.BuildFQName("node", sockStatSubsystem, "sockets_used"),
...
func (c *sockStatCollector) Describe(ch chan<- *prometheus.Desc) {
	// sockstat collector creates dynamic descriptors
} 