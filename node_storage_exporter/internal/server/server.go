...
import (
	...
	"math"
	"net/http"
	"node_storage_exporter/config"
	"node_storage_exporter/internal/exporter"
	_ "node_storage_exporter/internal/metrics"
	"node_storage_exporter/pkg/logger"
	"node_storage_exporter/pkg/ratelimit"
	"node_storage_exporter/pkg/utils"
	"os"
	"sync"
	"time"
...
func Run() {
	server := NewServer("node_storage_exporter", "1.0.0")
	err := server.SetUp()
	if err != nil {
		logrus.Fatalf("Failed to setup server: %v", err)
	}
...
// Part 2 commit for node_storage_exporter/internal/server/server.go
